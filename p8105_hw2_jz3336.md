...
The first pols\_moth.csv dataset contain information about national
...
be convert to character month in the full name. After clean up the data
set and rename it to pols\_month\_tidy, the dimension of the cleaned
data is \[822, 9\], where 822 is the number of rows and 9 is the number
of columns. The”president" variable is created to contain democratic
(dem) or republican (gop) at any given time. The year range of this data
set is 1947, 2015. The year range of the pols\_month\_tidy is the
largest among all three datasets. Therefore, when merging the three
datasets together, there will be some NA value associate with the year
1947 and 1948.

The second snp.csv dataset contain information about Standard & Poor’s
stock market index (S\&P). To clean up the data set, I separate the
“date” variable into “year”,“month”and day". Then, similar to treating
the pols\_month data, I create a month tibble and left\_joint it to the
snp. By doing this, all numeric month will be convert to character month
in the full name. After clean up the data set and rename it to
snp\_df\_tidy, the dimension of the cleaned data is \[787, 3\], where
787 is the number of rows and 3 is the number of columns. The year range
of this data set is 1950, 2015.

The third unemployment.csv dataset contain information about percentage
of unemployment in the associate month of selected years. To clean up
the data set, I used pivot\_longer to combine all months variable as
“month” while putting the associate unemployment percentages into the
“unemployment” column. Then, I create a month tibble which contain the
lower-case abbreviation of each month and month.name vectors. By
left\_jointing this month tibble to the unemployment\_df, all lower-case
abbreviation of each month will be convert to character month in the
full name. After clean up the data set and rename it to
